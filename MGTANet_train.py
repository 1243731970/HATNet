...
from model.BASE import MCCNet_VGG
...
# 改
image_root = './dataset/PV/train/image/'
gt_root = './dataset/PV/train/gt/'
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in train_loader_tqdm:
        ...
        s3_sig = torch.sigmoid(s3)
        s4_sig = torch.sigmoid(s4)

        # loss
        loss1 = CE(s1, gts) + IOU(s1_sig, gts) + binaryDiceLoss(s1_sig, gts)
        loss2 = CE(s2, gts) + IOU(s2_sig, gts) + binaryDiceLoss(s2_sig, gts)
        loss3 = CE(s3, gts) + IOU(s3_sig, gts) + binaryDiceLoss(s3_sig, gts)
        loss4 = CE(s4, gts) + IOU(s4_sig, gts) + binaryDiceLoss(s4_sig, gts)
    ...
    # 测试数据集配置
    test_image_root = './dataset/PV/test/image/'
    test_gt_root = './dataset/PV/test/gt/'
