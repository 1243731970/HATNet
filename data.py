...
class RandomScale(object):
    ...
    def __call__(self, img, mask):
        ...
        h = int(scale_amt * oh)
        w = int(scale_amt * ow)
        return img.resize((h, w), Image.BICUBIC), mask.resize((h, w), Image.NEAREST)

def cv_random_flip_2(img, label):
    # 定义翻转选项列表
...
class SalObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize):
        ...
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.tif') or f.endswith('.bmp')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.endswith('.jpg')
                    or f.endswith('.png') or f.endswith('.tif') or f.endswith('.bmp')]
    ...
    def __getitem__(self, index):
        ...
        image, gt = self.scale(image, gt)
        image, gt = cv_random_flip_2(image, gt)
